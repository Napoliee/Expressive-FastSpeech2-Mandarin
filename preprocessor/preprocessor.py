    ...
    def load_filelist_dict(self):
        ...
        with open(filelist_dir, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f.readlines()):
                ...
                # ESD Chinese格式：basename|text|speaker_id|dataset|default|emotion|arousal|valence
                basename = parts[0]
                if len(parts) >= 8:
                    # ESD Chinese format
                    text, speaker_id, dataset, default, emotion, arousal, valence = parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7]
                    aux_data = f"{speaker_id}|{text}|{emotion}"
                    filelist_dict[basename] = aux_data
                    emotions.add(emotion)
                    arousals.add(arousal)
                    valences.add(valence)
                elif len(parts) >= 6:
                    # 格式：clips/xxx.wav|文本|说话人|default|default|情感|0|0
                    text, speaker_id, emotion = parts[1], parts[2], parts[5]
                    aux_data = f"{speaker_id}|{text}|{emotion}"
                    filelist_dict[basename] = aux_data
                    emotions.add(emotion)
                    # 为了兼容，使用相同的情感值作为arousal和valence
                    arousals.add(emotion)
                    valences.add(emotion)
                else:
                    # 原始格式处理
                    aux_data = line.split("|")[3:]
