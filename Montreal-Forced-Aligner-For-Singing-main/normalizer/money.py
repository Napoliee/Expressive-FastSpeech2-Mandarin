...
num_pattern = r"(\d+[\d,]?(\.\d+)?)"  # 匹配包括小数点的数字
...
currency_pattern = r"({0}|{0}?({1}|{2}))".format(jinzhi_pattern,
                                                 money_mark_pattern,
                                                 money_currecy_pattern)
money_pattern = r"(({0}[多余几]|{0}[多余几]?{1})+)".format(num_pattern,
                                                     currency_pattern)


class Money(Normalizer):
    def __init__(self):
        ...
        if matchers:
            for matcher in matchers:
                ...
                text = text.replace(matcher[0],
                                    val_num_to_chn(target, self.system))
        return text
