...
from normalizer.digit import digit2chntext
    ...
    def _telephone_landline_normalize(self, text):
        ...
        while matchers:
            for matcher in matchers:
                target = matcher[0]
                target = self._telephone_number_normalize(target,
                                                          is_mobile=False)
    ...
    def _telephone_number_normalize(self, text, is_mobile=True):
        ...
        if matchers:
            for matcher in matchers:
                target = matcher[0]
                if is_mobile:
                    target = digit2chntext(target,
                                           alt_one=True,
                                           split_long=False)
                    target = target[:3] + SHORT_PAUSE_SYMBOL + target[
                        3:7] + SHORT_PAUSE_SYMBOL + target[7:11]
                else:
                    target = digit2chntext(target,
                                           alt_one=True,
                                           split_long=True)
